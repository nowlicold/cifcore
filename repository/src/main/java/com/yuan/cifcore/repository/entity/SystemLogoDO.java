/*
 * Bench Inc.
 * Copyright (c) 2004 All Rights Reserved.
 */
package com.yuan.cifcore.repository.entity;

// auto generated imports
import java.util.Date;
import com.baomidou.mybatisplus.annotation.TableId;
import com.baomidou.mybatisplus.annotation.IdType;
import java.io.Serializable;
import io.swagger.annotations.ApiModel;
import io.swagger.annotations.ApiModelProperty;
import lombok.Data;
import lombok.EqualsAndHashCode;
import com.baomidou.mybatisplus.annotation.TableName;

/**
 * A data object class directly models database table <tt>cif_system_logo</tt>.<br>
 * 系统头像
 * 
 *
 * This file is generated by <tt>bench-gen-dal</tt>, a DAL (Data Access Layer)
 * code generation utility specially developed for <tt>bench</tt> project.
 * 
 * PLEASE DO NOT MODIFY THIS FILE MANUALLY, or else your modification may
 * be OVERWRITTEN by someone else. To modify the file, you should go to 
 * directory <tt>(project-home)/biz/dal/src/conf/dalgen</tt>, and 
 * find the corresponding configuration file (<tt>tables/cif_system_logo.xml</tt>). 
 * Modify the configuration file according to your needs, then run <tt>bench-gen-dal</tt> 
 * to generate this file.
 *
 * @author cold
 */
@Data
@EqualsAndHashCode(callSuper = false)
@ApiModel(value="SystemLogoDO对象", description="系统头像")
@TableName("cif_system_logo")
public class SystemLogoDO implements Serializable  {
    private static final long serialVersionUID = 741231858441822688L;

	public static final String TABLE_NAME = "CIF_SYSTEM_LOGO";
	//===========column onstants=====
	/**
	 * This constant corresponds to db column <tt>id</tt>.<br>
     */
	 public static final String COLUMN_ID = "id";
	/**
	 * This constant corresponds to db column <tt>file_name</tt>.<br>
     */
	 public static final String COLUMN_FILE_NAME = "file_name";
	/**
	 * This constant corresponds to db column <tt>code</tt>.<br>
     */
	 public static final String COLUMN_CODE = "code";
	/**
	 * This constant corresponds to db column <tt>name</tt>.<br>
     */
	 public static final String COLUMN_NAME = "name";
	/**
	 * This constant corresponds to db column <tt>gmt_create</tt>.<br>
     */
	 public static final String COLUMN_GMT_CREATE = "gmt_create";
	/**
	 * This constant corresponds to db column <tt>gmt_modified</tt>.<br>
     */
	 public static final String COLUMN_GMT_MODIFIED = "gmt_modified";
	/**
	 * This constant corresponds to db column <tt>modifier</tt>.<br>
     */
	 public static final String COLUMN_MODIFIER = "modifier";
	/**
	 * This constant corresponds to db column <tt>group_id</tt>.<br>
     */
	 public static final String COLUMN_GROUP_ID = "group_id";
	/**
	 * This constant corresponds to db column <tt>order_number</tt>.<br>
     */
	 public static final String COLUMN_ORDER_NUMBER = "order_number";
	/**
	 * This constant corresponds to db column <tt>enabled</tt>.<br>
     */
	 public static final String COLUMN_ENABLED = "enabled";
	/**
	 * This constant corresponds to db column <tt>image_resource_id</tt>.<br>
     */
	 public static final String COLUMN_IMAGE_RESOURCE_ID = "image_resource_id";
	
	//========== properties ==========

	/**
	 * This property corresponds to db column <tt>id</tt>.<br>
     * 主键id
     */
	 @TableId(type = IdType.NONE)
	private long id;
	
	/**
	 * This property corresponds to db column <tt>file_name</tt>.<br>
     * 文件名
     */
	private String fileName;
	
	/**
	 * This property corresponds to db column <tt>code</tt>.<br>
     * 头像代码
     */
	private String code;
	
	/**
	 * This property corresponds to db column <tt>name</tt>.<br>
     * 头像名
     */
	private String name;
	
	/**
	 * This property corresponds to db column <tt>gmt_create</tt>.<br>
     * 创建时间
     */
	private Date gmtCreate;
	
	/**
	 * This property corresponds to db column <tt>gmt_modified</tt>.<br>
     * 修改时间
     */
	private Date gmtModified;
	
	/**
	 * This property corresponds to db column <tt>modifier</tt>.<br>
     * 修改人
     */
	private String modifier;
	
	/**
	 * This property corresponds to db column <tt>group_id</tt>.<br>
     * 分组id
     */
	private long groupId;
	
	/**
	 * This property corresponds to db column <tt>order_number</tt>.<br>
     * 排序值
     */
	private long orderNumber;
	
	/**
	 * This property corresponds to db column <tt>enabled</tt>.<br>
     * 是否有效
     */
	private boolean enabled;
	
	/**
	 * This property corresponds to db column <tt>image_resource_id</tt>.<br>
     * 图片源id
     */
	private String imageResourceId;
	

    //========== getters and setters ==========

}
